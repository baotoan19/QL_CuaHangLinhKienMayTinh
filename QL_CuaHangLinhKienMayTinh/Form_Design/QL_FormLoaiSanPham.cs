...
namespace QL_CuaHangLinhKienMayTinh.Form_Design
{
    public partial class QL_FormLoaiSanPham : Form
    {
        ...
        {
            con.Open();
            string tenLoai = txt_Search_tenLoai.Text;
            string searchQuery = "SELECT * FROM LOAISP WHERE TenLoai LIKE @tenLoai";
            SqlCommand cmd = new SqlCommand(searchQuery, con);
            cmd.Parameters.Add("@tenLoai", SqlDbType.NVarChar).Value = "%" + tenLoai + "%";
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
            dataGridView_LoaiSP.DataSource = dt;
            con.Close();
        }
        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            if (CheckTextBox())
            {
                ...
                string newTenSP = text_tenLoai.Text.Trim();

                LoaiSanPham loaisanpham = new LoaiSanPham(newMaSP, newTenSP);
        ...
        private void btn_Update_Click(object sender, EventArgs e)
        {
            ...
            if (CheckTextBox())
            {
                ...
                LoaiSanPham loaisanpham = new LoaiSanPham(newMaLSP, newTenLSP);
                loaisanpham.updateLoaiSanPham();
