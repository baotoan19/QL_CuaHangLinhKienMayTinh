...
namespace QL_CuaHangLinhKienMayTinh.Form_Design
{
    public partial class QL_FormIndex : Form
    {
        ...
        {
            SanPham sp = new SanPham();
            DataTable dt = sp.LoadTop5SanPhamBanChay(); // Lấy dữ liệu từ CSDL
            Series series = new Series("Số Lượng SP");
            chart1.Series.Clear();
            series.ChartType = SeriesChartType.Column; // Đặt loại biểu đồ là cột (bar chart)
            ...
            chart1.Series.Add(series); // Thêm series vào biểu đồ
        }
