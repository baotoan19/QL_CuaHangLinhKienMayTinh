    ...
    public partial class QL_FormNhaCungCap : Form
    {
        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            if (CheckTextBox())
            {
                ...
                NhaCungCap nhacc = new NhaCungCap(newMaNCC, newTenNCC, newSDT, newEmail, newDiaChi);
                int result = nhacc.insertNhaCungCap();
                if (result != -1)
                {
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
             ...
             MessageBoxDefaultButton.Button2) == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                NhaCungCap nhacc = new NhaCungCap(newMaNCC, newTenNCC, newSDT, newEmail, newDiaChi);
                int check = nhacc.deleteNhaCungCap(int.Parse(txt_maNCC.Text));
                if (check == -1)
                {
        ...
        private void btn_Update_Click(object sender, EventArgs e)
        {
            if (CheckTextBox())
            {
                ...
                NhaCungCap nhacc = new NhaCungCap(newMaNCC, newTenNCC, newSDT, newEmail, newDiaChi);
                nhacc.updateNhaCungCap(int.Parse(txt_maNCC.Text));
        ...
        {
            con.Open();
            string tenNCC = txt_Search_tenNCC.Text;
            string searchQuery = "SELECT * FROM NHACUNGCAP WHERE TenNCC LIKE @tenNCC";
        ...
        private void dataGridView_NCC_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txt_Email.Text = dataGridView_NCC.Rows[r].Cells[3].Value.ToString();
            txt_DC.Text = dataGridView_NCC.Rows[r].Cells[4].Value.ToString();
        }
